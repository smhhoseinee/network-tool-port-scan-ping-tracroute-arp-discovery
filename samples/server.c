...
void *serve_client(void* client_socket_void){
...
//		char* server_msg = malloc(sizeof(char) * MAX_MSG_OUT_LEN);
		puts("msg to client:\n");
		fgets(buffer,MAX_MSG_OUT_LEN,stdin);
...
int main(){
	...
	for(;;){
		...
		pthread_t tid;
		pthread_create(&tid, NULL, serve_client, (void *)&client_socket);
