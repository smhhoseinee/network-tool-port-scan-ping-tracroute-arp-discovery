...
//minumim input size which user chould input for packet
...
{
	int ttl_val=1 , msg_count=0, i, addr_len, flag=1,
...
int main(int argc, char *argv[])
{
	...
	pthread_t thread_id[number_of_threads-1];
	...
	for(int i=1; i<argc; i++){
		...
		if(arg_is_valid[i]){
			...
			//send_ping(sockfd, &addr_con, reverse_hostname,ip_addr, argv[i]);


			struct thread_send_ping_args *args = malloc (sizeof (struct thread_send_ping_args));
			args->ping_sockfd = sockfd;
			args->ping_addr = &addr_con;
			args->ping_dom = reverse_hostname;
			args->ping_ip = ip_addr;
			args->rev_host = argv[i];
...
*/
	...
	for(int i = 0; i < threads_counter; i++){
		int result_code = pthread_join(thread_id[i], NULL);
		assert(!result_code);
